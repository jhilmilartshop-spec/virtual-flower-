...
const FLOATERS = [
  { x: 17, y: 30, size: 13, delay: 0.0, color: "#FFD9D4" },
  { x: 84, y: 28, size: 11, delay: 2.1, color: "#B9D8EA" },
];
...
/* =========================================================
   HERO LANDSCAPE FLOWERS
   Flowers are planted in the landscape. Their stems remain
   anchored at the ground while each plant gently sways.
   ========================================================= */
function HeroLandscapeFlowers({ palette = "sunset" }) {
  const flowers = [
    { id: "lavender", left: 3, bottom: -7, scale: .42, delay: -1.4, speed: 5.8, opacity: .72 },
    { id: "cosmos", left: 10, bottom: -9, scale: .58, delay: -3.1, speed: 5.1, opacity: .88 },
    { id: "daisy", left: 19, bottom: -8, scale: .52, delay: -.7, speed: 5.7, opacity: .84 },
    { id: "poppy", left: 28, bottom: -10, scale: .63, delay: -2.5, speed: 4.9, opacity: .9 },
    { id: "rose", left: 38, bottom: -13, scale: .72, delay: -4.2, speed: 5.5, opacity: .98 },
    { id: "sunflower", left: 48, bottom: -13, scale: .68, delay: -1.9, speed: 6.2, opacity: .96 },
    { id: "peony", left: 58, bottom: -12, scale: .72, delay: -3.7, speed: 5.2, opacity: .98 },
    { id: "dahlia", left: 68, bottom: -10, scale: .63, delay: -1.1, speed: 5.9, opacity: .92 },
    { id: "forget", left: 77, bottom: -8, scale: .5, delay: -4.8, speed: 5.4, opacity: .82 },
    { id: "lily", left: 86, bottom: -8, scale: .58, delay: -2.2, speed: 6.1, opacity: .9 },
    { id: "gerbera", left: 94, bottom: -11, scale: .62, delay: -.9, speed: 5.0, opacity: .92 },
  ];

  return (
    <div className="hero-landscape-flowers" aria-hidden="true">
      <div className="hero-ground-grass">
        {Array.from({ length: 46 }).map((_, i) => (
          <span
            key={i}
            className="hero-grass-blade"
            style={{
              left: `${(i / 45) * 100}%`,
              height: `${26 + ((i * 17) % 42)}px`,
              animationDelay: `${-(i % 9) * .31}s`,
              transform: `rotate(${(i % 2 ? 1 : -1) * (8 + (i % 13))}deg)`,
            }}
          />
        ))}
      </div>
      ...
      {flowers.map((flower, index) => (
        <div
          key={`${flower.id}-${index}`}
          className="hero-planted-flower"
          style={{
            left: `${flower.left}%`,
            bottom: `${flower.bottom}%`,
            opacity: flower.opacity,
            animationDelay: `${flower.delay}s`,
            animationDuration: `${flower.speed}s`,
            zIndex: index > 3 ? 5 : 4,
          }}
        >
          <FlowerIllustration
    ...
    <main className="site-shell">
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .hero-ground-grass { position:absolute; left:0; right:0; bottom:-2px; height:110px; z-index:3; pointer-events:none; overflow:hidden; }
        .hero-grass-blade { position:absolute; bottom:0; width:2px; border-radius:100% 100% 0 0; background:linear-gradient(180deg,#6E875A,#263A2C); transform-origin:bottom center; animation:heroGrassSway 4.4s ease-in-out infinite; opacity:.72; }
        @keyframes heroGrassSway { 0%,100% { margin-left:0; } 50% { margin-left:4px; } }
        .hero-landscape-flowers { position:absolute; left:0; right:0; bottom:0; height:39%; z-index:4; pointer-events:none; overflow:hidden; }
        .hero-planted-flower { position:absolute; transform-origin:50% 100%; animation-name:heroPlantSway; animation-timing-function:ease-in-out; animation-iteration-count:infinite; will-change:transform; filter:drop-shadow(0 5px 5px rgba(24,24,22,.16)); }
        .hero-planted-flower .natural-flower { transform-origin:50% 100% !important; }
        @keyframes heroPlantSway { 0%,100% { transform:rotate(-1.8deg); } 23% { transform:rotate(1.5deg); } 47% { transform:rotate(-.8deg); } 71% { transform:rotate(2.3deg); } }
        ...
        @media (max-width: 760px) {
          ...
          .hero-landscape-flowers { height:34%; }
          .hero-planted-flower { animation-duration:6.2s !important; }
        }
      ...
      <section className="hero">
        ...
        <HeroLandscapeFlowers palette="sunset" />
        <FairyLights />
        <FloatingDecorations />
