...
const FLOATERS = [
  { x: 5, y: 18, size: 17, delay: 0.0, color: "#FFD9D4" },
  { x: 12, y: 31, size: 11, delay: 1.3, color: "#F5B7B2" },
  { x: 18, y: 54, size: 20, delay: 2.2, color: "#E7A6B8" },
  { x: 8, y: 72, size: 13, delay: .6, color: "#F7D4C7" },
  { x: 27, y: 17, size: 12, delay: 1.8, color: "#B9D8EA" },
  { x: 34, y: 69, size: 18, delay: 2.7, color: "#F3B6A1" },
  { x: 42, y: 10, size: 10, delay: .9, color: "#F9D9A5" },
  { x: 49, y: 78, size: 15, delay: 2.4, color: "#E9B6C2" },
  { x: 58, y: 17, size: 19, delay: 1.1, color: "#F7C8BA" },
  { x: 64, y: 65, size: 12, delay: 3.0, color: "#BBD6E5" },
  { x: 72, y: 27, size: 15, delay: .4, color: "#F2B4A5" },
  { x: 78, y: 49, size: 20, delay: 1.9, color: "#E4A3B8" },
  { x: 88, y: 17, size: 12, delay: .8, color: "#F9D7A3" },
  { x: 93, y: 34, size: 17, delay: 2.5, color: "#F3B9B0" },
  { x: 85, y: 72, size: 11, delay: 1.5, color: "#C2DDEA" },
  { x: 75, y: 82, size: 14, delay: 3.2, color: "#EAB0BE" },
  { x: 23, y: 83, size: 10, delay: 2.0, color: "#F7D2A7" },
  { x: 56, y: 88, size: 18, delay: .3, color: "#F0B2B4" },
];
...
function FairyLights() {
  const cords = [
    { top: "8%", rotate: -1.5, count: 14 },
    { top: "22%", rotate: 1.1, count: 16 },
    { top: "39%", rotate: -1.8, count: 13 },
    { top: "57%", rotate: 1.6, count: 15 },
    { top: "74%", rotate: -1.2, count: 12 },
  ];
  return (
    <div className="fairy-lights" aria-hidden="true">
      {cords.map((cord, cordIndex) => (
        <div key={cordIndex} className="fairy-cord" style={{ top: cord.top, transform: `rotate(${cord.rotate}deg)` }}>
          <div className="fairy-wire" />
          {Array.from({ length: cord.count }).map((_, i) => (
            <span key={i} className="fairy-bulb" style={{ left: `${2 + (i * 96) / (cord.count - 1)}%`, animationDelay: `${((i * 0.19) + cordIndex * 0.47) % 3.2}s` }} />
          ))}
        </div>
      ))}
    </div>
...
function AmbientMusic({ enabled, onToggle }) {
  ...
  const toggle = async () => {
    ...
    if (!AudioCtx) return;
    if (enabled) {
      if (gainRef.current) gainRef.current.gain.exponentialRampToValueAtTime(0.0001, audioRef.current.currentTime + 0.8);
      window.setTimeout(() => onToggle(false), 850);
      return;
    }
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .hero {
          ...
          background:
            radial-gradient(circle at 50% 59%, rgba(255,214,135,.98) 0 4.4rem, rgba(255,177,110,.48) 4.5rem 11rem, transparent 11.1rem),
            radial-gradient(ellipse at 50% 100%, #465141 0 17%, #343B39 18% 30%, transparent 31%),
            linear-gradient(180deg, #24162D 0%, #3E2342 18%, #813F55 40%, #D86661 58%, #F1A267 72%, #E9C08E 84%, #39463B 100%);
        }
        ...
        .fairy-cord { position:absolute; left:3%; right:3%; height:48px; transform-origin:center; }
        .fairy-wire { position:absolute; left:0; right:0; top:22px; height:2px; border-radius:999px; background:rgba(31,22,31,.72); box-shadow:0 1px 0 rgba(255,255,255,.08); }
        .fairy-bulb { position:absolute; top:17px; width:8px; height:12px; border-radius:50% 50% 55% 55%; background:#FFE99A; box-shadow:0 0 5px #FFF2B0, 0 0 13px rgba(255,211,105,.9), 0 0 27px rgba(255,173,86,.45), inset 1px 1px 2px rgba(255,255,255,.95); animation:fairyGlow 2.25s ease-in-out infinite; }
        .fairy-bulb:before { content:""; position:absolute; left:1.5px; top:-4px; width:5px; height:5px; border-radius:2px 2px 1px 1px; background:#40333C; }
        .fairy-bulb:after { content:""; position:absolute; inset:-8px; border-radius:50%; background:radial-gradient(circle, rgba(255,239,167,.28), transparent 65%); filter:blur(2px); }
        @keyframes fairyGlow { 0%,100% { opacity:.55; transform:scale(.84); } 35% { opacity:.9; transform:scale(1); } 50% { opacity:1; transform:scale(1.16); } 68% { opacity:.7; transform:scale(.93); } }
        ...
        @keyframes heartDrift { 0%,100% { translate:0 0; rotate:-4deg; opacity:.55; } 50% { translate:8px -13px; rotate:7deg; opacity:.95; } }
        ...
        .hero-divider { position:absolute; bottom:18%; left:50%; transform:translateX(-50%); z-index:3; display:flex; align-items:center; gap:10px; color:#FFD6B4; opacity:.8; }
