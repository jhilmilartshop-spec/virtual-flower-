...
function FairyLights() {
  const cords = [
    { top: "8%", bend: 12, count: 14 },
    { top: "24%", bend: 18, count: 16 },
    { top: "41%", bend: 10, count: 13 },
    { top: "59%", bend: 16, count: 15 },
    { top: "76%", bend: 12, count: 12 },
  ];
  return (
    <div className="fairy-lights" aria-hidden="true">
      {cords.map((cord, cordIndex) => (
        <svg key={cordIndex} className="fairy-cord-svg" viewBox="0 0 100 80" preserveAspectRatio="none" style={{ top: cord.top }}>
          <path className="fairy-wire-path" d={`M 0 18 Q 50 ${18 + cord.bend} 100 18`} />
          {Array.from({ length: cord.count }).map((_, i) => {
            const x = 3 + (i * 94) / (cord.count - 1);
            const t = x / 100;
            const y = 18 + cord.bend * 4 * t * (1 - t);
            const drop = 8 + ((i * 7 + cordIndex * 3) % 9);
            return (
              <g key={i} className="fairy-light-unit" style={{ animationDelay: `${((i * .17) + cordIndex * .41) % 3.1}s` }}>
                <line x1={x} x2={x} y1={y} y2={y + drop} className="fairy-drop" />
                <circle cx={x} cy={y + drop + 3} r="3.1" className="fairy-halo" />
                <circle cx={x} cy={y + drop + 3} r="1.75" className="fairy-bulb" />
              </g>
            );
          })}
        </svg>
    ...
    <main className="site-shell">
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .fairy-cord-svg { position:absolute; left:2%; width:96%; height:118px; overflow:visible; }
        ...
        .fairy-halo { fill:#FFE6A0; opacity:.35; filter:blur(3px); }
        ...
        @media (max-width: 430px) {
          ...
          .fairy-cord-svg { left:0; width:100%; height:92px; }
